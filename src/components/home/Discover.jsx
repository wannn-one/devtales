import { discoverAction, discoverContent } from "../../data/data"
  ...
  return (
    <div className="sticky top-[6rem]">
        <div className="border-b border-gray-400 pb-7">
            ...
            <button className="text-green-600 text-sm py-3 hover:text-customblack">See more topics</button>
        </div>
        <div className="flex items-center flex-wrap gap-3 leading-3 pt-8">
            {discoverAction.map((item, i) => (
                <button key={i} className="text-md text-customblack">{item}</button>
            ))}
        </div>
    </div>
  )
